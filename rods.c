...
#define ROD_LENGTH_INDEX 0
#define ROD_PRICE_INDEX 1
...
void input_lengths_and_prices(int lengths_and_prices[][CUT_OPTION_ENTRIES], int* number_of_options);
Cut_list* generate_cut_list_struct(int lengths_and_prices[][CUT_OPTION_ENTRIES], int number_of_options, int rod_length);
...
void display_lengths_and_prices(int lengths_and_prices[][CUT_OPTION_ENTRIES], int number_of_options); // FOR DEBUG
...
int main(int argc, char *argv[]){
    ...
    int rod_length; 
    /* Check that the input is a valid integer followed by no additional characters */
    int parsed_character_count;
    if (sscanf_s(argv[1], "%d%n", &rod_length, &parsed_character_count) != 1 || argv[1][parsed_character_count] != '\0' || rod_length <= 0) {
        printf("Failed to provide a valid rod length\n");
        return 1;
    }

    int lengths_and_prices[MAX_CUT_OPTIONS][CUT_OPTION_ENTRIES] = {0}; 
    int number_of_options = 0; 
    input_lengths_and_prices(lengths_and_prices, &number_of_options); 
    // display_lengths_and_prices(lengths_and_prices, number_of_options); // DEBUG 
...
void input_lengths_and_prices(int lengths_and_prices[][CUT_OPTION_ENTRIES], int* number_of_options){
    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL) {
        if (sscanf_s(buffer, "%d, %d", &length, &price) == 2) {
            if (length > 0 && price > 0) {
                lengths_and_prices[*number_of_options][ROD_LENGTH_INDEX] = length;
                lengths_and_prices[*number_of_options][ROD_PRICE_INDEX] = price;
                (*number_of_options)++;
            } 
            
            if (length <= 0 || price <= 0)
                printf("Invalid input ignored (non-positive)\n"); 
...
void display_lengths_and_prices(int lengths_and_prices[][CUT_OPTION_ENTRIES], int number_of_options){

    printf("\nLengths and Prices Entered\n");
    printf("=========================\n\n");

    for (int current_cut = 0; current_cut < number_of_options; current_cut++){
       printf("Length: %d, Price: %d\n", lengths_and_prices[current_cut][ROD_LENGTH_INDEX], lengths_and_prices[current_cut][ROD_PRICE_INDEX]);   
    }
}
...
Cut_list* generate_cut_list_struct(int lengths_and_prices[][CUT_OPTION_ENTRIES], int number_of_options, int rod_length){
    ...
    for (int current_cut = 0; current_cut < number_of_options; current_cut++) {
        optimal_cut_list->cut[current_cut].cut_length = lengths_and_prices[current_cut][ROD_LENGTH_INDEX];
        optimal_cut_list->cut[current_cut].cut_price = lengths_and_prices[current_cut][ROD_PRICE_INDEX];
